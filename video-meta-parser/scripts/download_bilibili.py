...
功能:
    - 优先使用 yt-dlp 下载（自动处理 DASH 音视频合并）
    - yt-dlp 失败时回退到 B站 API + ffmpeg 手动合并
...
def download_with_api(bvid: str, cid: int, save_path: str,
    ...
    """回退方案：通过 B站 API 获取 DASH 流 + ffmpeg 合并"""
    ffmpeg = shutil.which('ffmpeg')
    if not ffmpeg:
        print("      ffmpeg 未安装，无法使用 API 回退")
        return False

    # 获取 DASH 流地址
    resp = session.get(BILIBILI_API_PLAY, params={
        "bvid": bvid, "cid": cid,
        "qn": 80, "fnval": 16, "fourk": 1,
    })
    data = resp.json()
    if data.get('code') != 0:
        print(f"      playurl API 错误: {data.get('message')}")
        return False
    ...
    print("      下载视频流 ...")
    _download_stream(video_url, video_tmp, headers)
    print("      下载音频流 ...")
    _download_stream(audio_url, audio_tmp, headers)
...
def _download_stream(url: str, save_path: str, headers: dict) -> None:
    """下载单个 DASH 流（带进度条）"""
...
def download_video(bvid: str, cid: int, save_path: str,
    ...
    """优先 yt-dlp，失败回退 API+ffmpeg"""
    print("      尝试 yt-dlp ...")
    if download_with_ytdlp(bvid, save_path, cookie_file):
        return
    print("      回退 API+ffmpeg ...")
    if download_with_api(bvid, cid, save_path, session):
        return
    raise RuntimeError("所有下载方式均失败")
